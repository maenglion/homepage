// SLDA §7·§14 — 전처리(1단) 프롬프트 발급 Edge Function (엄격 모드)
//
// 목적: §7 전처리 프롬프트를 코드에서 완전히 분리해 서버에서 "발급"한다.
//   - 프롬프트 본문은 이 함수 소스에 없다. slda_prompts 테이블에서 active=true 인 것을 읽어 반환한다.
//     (레포가 public 이라 함수 소스도 노출되므로, 본문은 DB(테이블)에만 둔다.)
//   - 1단 프롬프트(라벨화·마스킹·병합)만 발급한다. 2단(정규화)·3단(채점)은 여기서 절대 노출하지 않는다.
//   - §14.4 발급 세션: 유효한 세션 토큰(slda-start-session 발급)이 있어야만 발급한다.
//     봇·스크래퍼 차단. 토큰 없음/만료 → 401.
...
//        401 { error: "session_required" }   ·   404 { error: "prompt_unavailable" }
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  // 프롬프트 본문은 DB(slda_prompts)에서만. 함수 소스에 없다.
  const { data, error } = await db
    .from("slda_prompts")
    .select("body, version")
    .eq("model", model)
  ...
  if (error) return json({ error: "prompt_lookup_failed", detail: error.message }, 500);
  if (!data || !data.body) return json({ error: "prompt_unavailable" }, 404);

  return json({ model, version: data.version, prompt: data.body });
});
